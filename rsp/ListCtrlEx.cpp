...
void CListCtrlEx::ShowSubCtrl(int nItem, int nSubItem,DWORD dwCtrlType/* = COLUMN_STYLE_EDIT*/)
{
	...
	if(dwCtrlType==COLUMN_STYLE_COMBOBOX)
	{
		...
		CListCellCombo * pCombo = new CListCellCombo(nItem,nSubItem,strCulText);
		pCombo->Create(WS_CHILD|WS_VISIBLE|WS_VSCROLL|CBS_DROPDOWNLIST|CBS_OWNERDRAWFIXED|CBS_HASSTRINGS,rc,this, 1);
...
{
	if (CColorSelectComboBox::OnCreate(lpCreateStruct) == -1)
		return -1;
...
void CListCellCombo::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{

	// TODO:  添加您的代码以绘制指定项
	CColorSelectComboBox::DrawItem(lpDrawItemStruct);
}

void CListCellCombo::MeasureItem(LPMEASUREITEMSTRUCT lpMeasureItemStruct)
{

	// TODO:  添加您的代码以确定指定项的大小
	CColorSelectComboBox::MeasureItem(lpMeasureItemStruct);
}
