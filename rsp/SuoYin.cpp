...
GDALColorEntry colorEntry[4];
//ImageShow* SY;

int SIndex=0;


BYTE ColorsTable[12][256][4] = 
{
	{	// 常规灰度编码
...
IMPLEMENT_DYNAMIC(SuoYin, CDialogEx)

SuoYin::SuoYin(CWnd* pParent /*=NULL*/)
	: CDialogEx(SuoYin::IDD, pParent)
{
...
void SuoYin::DoDataExchange(CDataExchange* pDX)
{
	CDialogEx::DoDataExchange(pDX);
}


BEGIN_MESSAGE_MAP(SuoYin, CDialogEx)
	ON_LBN_SELCHANGE(IDC_LIST_ColorTable, &SuoYin::OnLbnSelchangeListColortable)
	ON_BN_CLICKED(IDCANCEL, &SuoYin::OnBnClickedCancel)
	ON_BN_CLICKED(IDOK, &SuoYin::OnBnClickedOk)
END_MESSAGE_MAP()
...
BOOL SuoYin::OnInitDialog()
{
	...
	if (nBandCount>1)
	{
		AfxMessageBox("请输入单波段影像");
		return FALSE;
	}
	CString a[12]={ "------------------------1常规灰度编码----------------------",
					"------------------------2逆灰度编码-------------------------",
					"------------------------3红色饱和度编码-------------------",
					"------------------------4绿色饱和度编码-------------------",
					"------------------------5蓝色饱和度编码-------------------",
	...
	Listbox=(CListBox*)GetDlgItem(IDC_LIST_ColorTable);
	
	for (int i=0;i<12;i++)
	{
		Listbox->AddString(a[i]);
	}
...
void SuoYin::OnLbnSelchangeListColortable()
{
	SIndex=Listbox->GetCurSel();
	// TODO: 在此添加控件通知处理程序代码
}


void SuoYin::OnBnClickedCancel()
{
	// TODO: 在此添加控件通知处理程序代码
//	SY->DestroyWindow();
	DestroyWindow();
...
void SuoYin::OnBnClickedOk()
{
	// TODO: 在此添加控件通知处理程序代码
	flag1=2;
	//SY=new ImageShow;
	//SY->Create(IDD_IMAGE_SHOW);
	//SY->ShowWindow(SW_SHOW);
