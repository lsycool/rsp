...
BOOL CDlgLinerPara::LinerTrans(unsigned char*lpSrc, int BandCount, LONG lWidth, LONG lHeight, FLOAT fA, FLOAT fB)
{
	// 循环变量
	LONG	i;
	LONG	j;
	...
	for (int k=0;k<BandCount;k++)
	{
		...
		for(i = 0; i < lHeight; i++)
		{
			...
			for(j = 0; j < lWidth; j++)
			{
				...
				// 线性变换
				fTemp = fA * lpSrc[lWidth*lHeight*k+i*lWidth+j] + fB;

				// 判断是否超出范围
				if (fTemp > 255)
				{
					// 直接赋值为255
					lpSrc[lWidth*lHeight*k+i*lWidth+j] = 255;
				}
				else if (fTemp < 0)
				{
					// 直接赋值为0
					lpSrc[lWidth*lHeight*k+i*lWidth+j] = 0;
				}
				else
				{
					// 四舍五入
					lpSrc[lWidth*lHeight*k+i*lWidth+j] = (unsigned char) (fTemp + 0.5);
				}
			}
		}	
	}
